      ...
      >
        <Card bordered={false}>
          <DescriptionList size="small" col={2}>
            <Description style={{ wordBreak: 'break-word' }} term="员工ID">{values.userId || '-'}</Description>
            <Description style={{ wordBreak: 'break-word' }} term="员工名称">{values.userName || '-'}</Description>
            <Description style={{ wordBreak: 'break-word' }} term="密码">{values.pwd || '-'}</Description>
            <Description style={{ wordBreak: 'break-word' }} term="联系邮箱">{values.email || '-'}</Description>
            <Description style={{ wordBreak: 'break-word' }} term="联系地址">{values.address || '-'}</Description>
            <Description style={{ wordBreak: 'break-word' }} term="入职时间">{values.hiredate || '-'}</Description>
            <Description style={{ wordBreak: 'break-word' }} term="是否管理员 0 否 1 是">{values.isAdmin || '-'}</Description>
